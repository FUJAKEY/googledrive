...
> Полную документацию REST API, сценарии использования API-ключей и примеры `curl` смотрите в [docs.md](docs.md).
...
- `GET /s/:token` — публичный просмотр (HTML) или JSON-ответ с метаданными при заголовке `Accept: application/json`.
- `GET /s/:token/download` — скачивание расшаренного файла.
- `GET /s/:token/archive` — скачивание архива ZIP для расшаренной папки.
